...
for _splitname in splitnames:
    ...
    for _fn in _data:
        ...
        if _fn == "":
            continue
        filenamesBySplit[_splitname].append(os.path.basename(_fn).replace(".jpg", ""))
    ...
    for _splitname in splitnames:
        if x in filenamesBySplit[_splitname]:
            if _x_split is not None:
                print(
                    "Duplicate filenames for {}, found in {} and {}".format(
                        x, _x_split, _splitname
                    )
                )
                exit(1)
            else:
                _x_split = _splitname
    filenamesWithSplitName.append((x, _x_split))
